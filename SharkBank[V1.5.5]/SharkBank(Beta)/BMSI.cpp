...
Bank_MSI::Bank_MSI(std::string username_, double Money_)
{
    std::cout << "[Shark Init]" << " running" << std::endl 
...
void Bank_MSI::Savings(double sav)
{
    if (sav < 0) {
        ...
        Money += sav;
        std::cout << std::setprecision(2) << "Bank account balance : " << Money << " RMB." << std::endl << std::endl;
...
bool Bank_MSI::Withdrawal(double wit) 
{
    using std::cout;
    if (wit < 0) {
        cout << "Error : The withdrawal amount is less than 0 RMB." << std::endl << std::endl;
        Sleep(2000);
...
void Bank_MSI::Estimate_Loan(double loan_money, int loan_time) const
{
    ...
    else
    {
        ...
        std::cout << std::setprecision(3)
            << "Loan " << loan_money << " RMB." << std::endl
            << lmli << " RMB should be repaid after " << loan_time << " year(s)." << std::endl << std::endl;
    }
    Sleep(2000);
}
...
void Bank_MSI::Estimate_asset(int years) const
{
    ...
    else
    {
        ...
        std::cout << std::setprecision(3) << "Estimated in " << years << " years your assets are : " << expected_asset << " RMB." << std::endl << std::endl;
    }
    Sleep(2000);
}
